...
## 리뷰는 별도 사이트로 분리했다 (2026-09-16)

리뷰 파이프라인은 **`zengenetics/reviews/`** 로 옮겼다. 이 폴더(카페24 업로드용
프로토타입)와 배포 경로가 다르기 때문이다 — 리뷰는 `review.zengenetics.co.kr`
서브도메인에 GitHub Pages 로 **자동 배포**된다.

배경: 상품페이지 리뷰는 알파리뷰 JS 위젯이 로드 후 주입해서 원본 HTML 에 본문이
0자다. JS 를 실행하지 않는 크롤러 — AI 답변을 만드는 쪽 대부분 — 는 리뷰를 아예
못 읽는다. 게다가 카페24 가 박아둔 JSON-LD `reviewCount` 는 **419** 인데 알파리뷰
실제 보유량은 **26,969** 건이다. 구글은 실제의 1/64 로 알고 있다.

운영 절차·심의 설계·API 실측 한계는 `zengenetics/reviews/README.md` 에 있다.
